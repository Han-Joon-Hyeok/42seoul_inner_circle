...
char	*split_newline(t_node *p_node, int i)
{
	...
	if (newline == NULL)
		return (NULL);
	newline = ft_memcpy(newline, p_node->backup, i);
	newline[i] = '\0';
	if (BUFFER_SIZE - i > 1)
	{
		...
		if (next == NULL)
			return (NULL);
		next = ft_memcpy(next, &p_node->backup[i + 1], BUFFER_SIZE - i - 1);
...
char	*clear_backup(t_node **p_head, int fd)
{
	t_node	*p_node;
	char	*newline;
